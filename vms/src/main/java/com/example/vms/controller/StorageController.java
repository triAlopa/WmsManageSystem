...
//        if(userService.save(user)) return new Result(200, "success",1,user);
    ...
    @PostMapping("/update")
    private Result update(@RequestBody Storage storage) {
...
// 注解：指定HTTP POST请求的URL路径为"/listPage"
    public Result listPage(@RequestBody queryPageParam query) {
        ...
        // 创建一个Page对象，用于分页查询
        Page<Storage> storagePage = new Page<>();
        // 设置每页显示的记录数，从查询参数中获取
        storagePage.setSize(query.getPageSize());
        // 设置当前页码，从查询参数中获取
        storagePage.setCurrent(query.getPageNum());
        ...
        // 执行分页查询，返回分页结果
        IPage<Storage> result = storageService.page(storagePage,lambdaQueryWrapper);
