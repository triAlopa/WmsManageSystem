...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        name: [
          {required: true, message: '请输入仓库分类', trigger: 'blur'},
  ...
  methods: {
    ...
    del(id) {
      this.$axios.get(this.$httpUrl +'/goodstype/delete?id='+id, this.form).then(res => res.data).then(res => {
        if (res.code==200) {
          this.$message({
            message: '删除成功',
            type: 'success'
...
<template>
  <div>
    <div style="margin-bottom: 5px;">
      <el-input placeholder="请输入分类名" v-model="name" suffix-icon="el-icon-search" style="width: 200px;"
